...
validate:
	...
	$(UV) run python -c "from pathlib import Path; text=Path('output/index.html').read_text(); assert 'uk_2026Q2_v1.html' in text; assert '6</strong><span>country dashboards' in text"
	$(UV) run python -c "from pathlib import Path; text=Path('index.html').read_text(); assert 'Macro Dashboard Archive' in text and 'output/china_2026Q2_v1.html' in text and 'output/uk_2026Q2_v1.html' in text"
